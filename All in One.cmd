...
set "AZ_KEYS=%~1"
set "AZ_ERRORLEVEL="
for /f "usebackq delims=" %%E in (`powershell -NoProfile -Command "$valid='%AZ_KEYS%'.ToUpperInvariant(); $quiet=[Diagnostics.Stopwatch]::StartNew(); while($quiet.ElapsedMilliseconds -lt 800){ if([Console]::KeyAvailable){ [void][Console]::ReadKey($true); $quiet.Restart() } else { Start-Sleep -Milliseconds 25 } }; while($true){ $k=[Console]::ReadKey($true); $ch=[char]::ToUpperInvariant($k.KeyChar); if($k.Key -match '^D([0-9])$'){ $ch=$Matches[1][0] } elseif($k.Key -match '^NumPad([0-9])$'){ $ch=$Matches[1][0] } elseif($k.Key -match '^[A-Z]$'){ $ch=$k.Key.ToString()[0] }; $idx=$valid.IndexOf([string]$ch); if($idx -ge 0){ [Console]::Write($idx+1); exit } }"`) do set "AZ_ERRORLEVEL=%%E"
